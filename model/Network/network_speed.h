...
namespace knet {
    ...
    private:
        static int depth;
        static client_info client;

        static int     num_thread;
        static double  latency, speed, download_speed, upload_speed;
        static int     dsize, sindex;

        static char server_url[URL_LENGTH_MAX], ext[UPLOAD_EXT_LENGTH_MAX];
        static server_info servers[MAX_CLOSEST_SERVER_NUM];
    ...
    return size * nmemb;
        ...
        static int test_download(char *p_url, int num_thread, int dsize, char init)
        {
            ...
            for ( i = 0; i < num_thread; i++) {
                ...
                paras[i].result = 0;
                error = pthread_create(&paras[i].tid, nullptr, do_download, (void*)&paras[i]);
            ...
            for (i = 0;i < num_thread; i++) {
                pthread_join(paras[i].tid, nullptr);
                sum += paras[i].result;
            }
        ...
        static int do_upload(void* _para)
        {
            CURL *curl;
            CURLcode res;
            struct thread_para* para = (struct thread_para*)_para;
        ...
        static int test_upload(char *p_url, int num_thread, long size, char *p_ext, char init)
        {
            ...
            for ( i = 0; i < num_thread; i++) {
                ...
                //printf("szeleft = %ld\n", paras[i].upload_size);
                error = pthread_create(&paras[i].tid, nullptr, (void * _Nullable (* _Nonnull)(void * _Nullable))do_upload, (void*)&paras[i]);
        ...
        static int get_client_info(struct client_info *p_client)
        {
            CURL *curl;
            CURLcode res;
            XML_Parser xml = XML_ParserCreate(nullptr);
            struct web_buffer web;
            char* p = nullptr;
            int i;
